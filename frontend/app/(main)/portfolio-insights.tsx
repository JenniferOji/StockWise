...
export default function PortfolioInsights() {
	...
	// sectorData groups shares by sector - acc is an object with sector names as keys and total shares as values
	const sectorData = useMemo(() => {
		const data = stocks.reduce((acc, stock) => {
			// if the sector key doesnt exist yet - initialise it to 0
			if (!acc[stock.sector]) {
				acc[stock.sector] = 0
			}
			// calculates the total amount of shares per sector
			acc[stock.sector] += (stock.quantity || 0)
			return acc
		}, {} as Record<string, number>); // initial value is an empty object
	...
	// total allocation percentage for the page header 
	const total = useMemo(() => sectorAllocations.reduce((s, c) => s + c.allocation, 0), [sectorAllocations])
	// measures the available width with paddign subtracted so the chart fits the card
	...
	// useMemo to avoid recalculating unless the stocks change
	...
	return (
		<SafeAreaView style={styles.container}>
							...
							<View style={styles.legendContainer}>
								{chartData.map((item, index) => (
									<TouchableOpacity 
										...
										// when a sector is clicked it sets it as the selected sector or deselects if already selected
			...
			{/* When a sector label is clicked on it displays the pop up  */}
